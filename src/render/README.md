...
| File | What |
|---|---|
| `Viewport.ts` | Scene, lights, black bg, ghost cube, render-on-demand loop. Subscribes to `Editor` `voxels`/`palette` events and marks chunks dirty. `pick(grid, clientX, clientY)` → DDA `Hit`. |
